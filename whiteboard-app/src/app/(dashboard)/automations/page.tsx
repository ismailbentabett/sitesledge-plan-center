    ...
    <div className="p-6 max-w-6xl">
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          description="Document your GHL automation workflows"
          action={{ label: 'Add Automation', onClick: () => router.push('/automations/new') }}
        />
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="border-b bg-muted/50">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-medium text-muted-foreground">GHL Workflow</th>
            ...
            <tbody>
              {filtered.map((template) => (
                <tr key={template.id} className="border-b last:border-0 hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3 text-muted-foreground truncate max-w-xs">{template.purpose || '-'}</td>
                  <td className="px-4 py-3 text-muted-foreground">{template.packageLevel || '-'}</td>
                  <td className="px-4 py-3 text-muted-foreground">{template.ghlWorkflowName || '-'}</td>
                  <td className="px-4 py-3"><StatusBadge label={template.status} variant={statusColors[template.status] || 'default'} /></td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex justify-end gap-2">
                      <button onClick={() => router.push(`/automations/${template.id}`)}
