  ...
  const saveRename = async (boardId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setBoards(
          boards.map((b) =>
            b.id === boardId ? { ...b, title: renameTitle.trim() } : b
          )
        )
      }
    } catch (error) {
  ...
  const togglePublic = async (board: Board) => {
    try {
      ...
      if (res.ok) {
        setBoards(
          boards.map((b) =>
            b.id === board.id ? { ...b, isPublic: !b.isPublic } : b
          )
        )
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {error && (
          <div className="p-3 mb-6 rounded-lg bg-destructive/10 text-destructive text-sm">
            {error}
          </div>
        )}
        ...
        {boards.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {boards.map((board) => (
              <div
                key={board.id}
                className="group relative border rounded-xl bg-card hover:shadow-md transition-all"
              >
                <div className="p-5 space-y-4">
                    ...
                    {renamingId === board.id ? (
                      <div className="flex-1">
                        <input
                          ...
                          className="w-full px-2 py-1 text-sm font-semibold border border-input rounded-md bg-background focus:outline-none focus:ring-2 focus:ring-ring"
                          autoFocus
                        />
                      </div>
                    ) : (
                  ...
                  {board.isPublic && (
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => copyShareLink(board.publicId)}
                  ...
                  <div className="flex gap-2 pt-1">
                    <Link
                      ...
                      className="flex-1 inline-flex items-center justify-center px-3 py-2 text-sm font-medium text-primary-foreground bg-primary rounded-md hover:bg-primary/90 transition-colors"
                    >
                      Open
                    </Link>
                    <button
                      onClick={() => togglePublic(board)}
                      className="inline-flex items-center justify-center px-3 py-2 text-sm border border-input rounded-md hover:bg-accent transition-colors"
                    >
